...
class HomePage extends Component {
  ...
  componentDidMount() {
    ...
    this._asyncRequest = this.props.firebase.getCities().then(locationData => {
    ...
    this._asyncRequest = this.props.firebase.getAllHotels().then(result => {
      this._asyncRequest = null;
      this.setState(
        {
          allHotels: result,
  ...
  handleSearch = e => {
    ...
    if (this.state.search.location.hasOwnProperty("data")) {
      ...
      if (city && state && country) {
        searchedHotels = this.state.allHotels.filter(
          hotel =>
            hotel.data.address.city
              .toLowerCase()
              .includes(city.toLowerCase()) &&
            hotel.data.address.state
              .toLowerCase()
              .includes(state.toLowerCase()) &&
            hotel.data.address.country
              .toLowerCase()
    ...
    if (this.state.search.roomType) {
      let roomTypePrices = [];
      searchedHotels = searchedHotels.filter(hotel => {
        let roomTypeOfHotel = hotel.data.room_types.filter(
          room_type => room_type.type === this.state.search.roomType
        );
    ...
    this.setState(
      {
        filter: {
          ...this.state.filter,
          price:
            ((this.state.filter.maxPrice - this.state.filter.minPrice) * e.x) /
              100 +
  ...
  handleFilter = type => {
    ...
    //filter by price
      ...
      filteredHotels = searchedSortedHotels.filter(hotel => {
        const roomTypeData = hotel.data.room_types.filter(
          roomType => roomType.type === this.state.search.roomType
        );
